...
public class PasskeyController {
  ...
  @GetMapping List<Map<String,Object>> list(@AuthenticationPrincipal Jwt jwt){String email=String.valueOf(jwt.getClaim("email"));return db.sql("SELECT encode(c.credential_id,'hex') credential_id,c.label device_name,c.created,c.last_used FROM user_credentials c JOIN user_entities u ON u.id=c.user_entity_user_id WHERE lower(u.name)=lower(:e) ORDER BY c.created DESC").param("e",email).query().listOfRows();}
  @DeleteMapping("/{credentialId}") ResponseEntity<Void> remove(@AuthenticationPrincipal Jwt jwt,@PathVariable String credentialId){String email=String.valueOf(jwt.getClaim("email"));int deleted=db.sql("DELETE FROM user_credentials c USING user_entities u WHERE c.user_entity_user_id=u.id AND lower(u.name)=lower(:e) AND c.credential_id=decode(:id,'hex')").param("e",email).param("id",credentialId).update();return deleted==1?ResponseEntity.noContent().build():ResponseEntity.notFound().build();}
}
