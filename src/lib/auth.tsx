...
interface AuthContextValue {
  ...
  signIn: (identifier: string, password: string, rememberDevice: boolean) => Promise<{ mfa: boolean; identifier: string }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function signIn(identifier: string, password: string, rememberDevice: boolean) {
    ...
    try {
      ...
      if (result.additionalVerificationRequired) return { mfa: true, identifier }
      setAccessToken(result.accessToken); setUser(normalize(result.user)); return { mfa: false, identifier }
    } catch (error) {
      if (!(error instanceof BackendUnavailableError)) throw error
      if (identifier.trim().toLowerCase() !== DEMO_EMPLOYEE.email.toLowerCase() || password !== DEMO_EMPLOYEE.password) {
  ...
  async function signInWithPasskey() {
    ...
    if (!optionResponse.ok) throw new Error("Passkey sign-in could not start.")
    ...
    if (!verification.ok) throw new Error("Biometric verification failed. Use your password instead.")
